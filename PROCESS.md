...
four skills built before it, in order (`98b6fab`).
...
can't quietly erode it (`f817016`). Nothing enforced the three weights
...
the gap with a test instead (`d319cad`).
...
course arguing AI slop is dishonest shouldn't use on itself (`0c77b05`). I
...
`/specimens/` page, still hand-authored SVG (`658d21f`, `1ea2196`) — plus a
fact-check pass fixing an inverted academic-integrity clause (`c8bfd82`) and
recalibrating real-world claims against sources (`ff18288`, `e19e549`).
...
(`d41fb13`), every lecture and session (`ba1851d`), the home page roadmap
(`693d4b4`), and staff portraits (`ed53ee1`). A follow-up split it further:
lectures and sessions had shared one photo per week, reading as duplicates,
so sessions got their own distinct directory (`67f533e`).
...
written by me directly (`0c0fb46`). This doesn't retroactively apply to
